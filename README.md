...
- 공개 포트: TCP **9001** (gate), TCP **9002** (finale), UDP **동적** (20000~24900 범위, 세션별 계산)
...
```
 ...
 participant ── TCP 9001 ──▶│ Gate (Layer A+B 자동항)       │
                            ...
                            └──────────────┬───────────────┘
                                           ...
                                           ▼
                            ┌──────────────────────────────┐
 participant ── UDP ?? ────▶│ Transient UDP (15s TTL)      │◀─ Glob (Layer C)
                            │  port = 20000 + L3*7          │
                            │  sid → ticket(8B) 1회         │
                            └──────────────┬───────────────┘
                                           │ sid+tk
...
- 모든 관측값(응답 길이, 지연 클래스, echo, FIN 시점)은 **서버가 의도적으로 통제하는 값**이다. 커널/경로 우연값에 의존하지 않는다.
...
| UDP 트랜지언트 | 세션별 1개, 15초 TTL, sid 1회 발급 |
| `ip_last_udp` | UDP 포트 개설 IP당 0.3s 스페이싱 |
...
|---|---|---|---|
| connect | — | 무응답 대기 | 침묵 |
| probe | 16B 수신(events≥3) | `jit(dA)` 대기 → blob0, `jit(dB)` 대기 → blobT(24B) | blob0 길이 `24+a`, **delay 쌍 (80,200) 또는 (200,80)ms** |
...
6. **UDP port**: `20000 + L3*7` (L3 관측값에서 계산)
...
| decoy | 겉모습 | 분리 방법 |
|---|---|---|
| 단일 write probe | 이후 모든 단계 정상 진행 | 배너 δ≈0 (페어 지연 동일), HC δ≈0, UDP 포트 침묵 |
...
genuine 세션이 half-close 단계에 도달하면 `udp_open_for()`가 `20000+L3*7` 포트를 바인드하고 15초 수신 스레드를 시작. 올바른 sid 수신 시 `tk=os.urandom(8)` 1회 응답. decoy/만료/중복은 **침묵**.

## 10. global state manager

`Glob` (threading.Lock 보호): 세션 테이블·run_counter·last_leak·UDP 상태·IP 스페이싱·동시성 세마포어. TCP/UDP가 동일 객체를 공유 → cross-protocol state.

## 11. transient port/service

`port = 20000 + L3*7` (L3는 응답 길이로 관측 가능), TTL 15초, 바인드 충돌 시 L3 재추첨(응답 전에 결정되므로 정합성 유지). 전체 포트 스캔 대신 **관측값 기반 단일 후보** 접속이 정답 루트.

## 12. 동시 접속 처리

thread-per-connection + 전역 세마포(64) + **IP당 동시 6** (BoundedSemaphore, 120초 대기 후 드롭). 초과 접속은 큐잉되며, 대기 중 probe가 병합되어 decoy화 — 남발이 자연히 저품질 세션으로 귀결(§14).

## 13. timeout 정책

probe 유휴 30s, 단계별 recv 2~30s, UDP 15s, 세션 레코드 10min GC, C2 입력 15s, finale 교환 10s. 조용히 종료(ERR 남발 없음).

## 14. anti-bruteforce

1. token 오답 → challenge 리셋(누적 학습만 가능, 연결당 5회 실패 시 poisoned=ERR 루프)
2. UDP tk 1회성 + 15초 TTL + IP당 0.3s 스페이싱
3. finale `sid+tk` 1회
...
`challenge/Dockerfile` + `docker-compose.yml`: python:3.12-slim, 9001/9002 tcp + 20000-25000/udp 매핑, 플래그는 env `BTA_FLAG` (빌드 arg로도 주입 가능). Dreamhack에서 컨테이너 기반 배포 그대로 동작.
...
불필요. 트랜지언트 포트는 프로세스가 직접 바인드하고 TTL로 닫힘. 방화벽이 있다면 9001/9002/tcp, 20000-25000/udp만 아웃바운드→인바운드 허용.

## 18. 서버 코드 구조

```
server.py
├─ 상수/플래그 청킹 (split_flag, keystream)
├─ Sess (연결별 hidden state) / Glob (전역 상태)
├─ read_probe / echo_phase / read_exact   ← Layer A·B 측정
├─ udp_open_for / udp_loop               ← 트랜지언트 서비스
...
6. 응답1 길이 L3 ↔ 열린 UDP 포트를 여러 세션에서 비교 → `port = 20000 + L3*7` 선형 관계 확립
...
| L3 값 변동 | 열린 UDP 포트 | 선형 사상 | 회귀로 `20000+7·L3` 확정 |
| blob 비ASCII | leak·크리브 | XOR 체인 | `LR{` 확인 |

## 21. 최종 solver

`solve/solver.py` — connection 생성, 지연 측정·페어 분류, token 계산, 세그먼트 분할 전송, half-close, UDP 포트 계산, finale delta 분류, leak 조립, 4세션 transcript 저장, `decode()` 복원. `--probe N`은 intended RE 도구(분포 분석). Scapy 불필요 — 단, tcpdump/wireshark로 δ·길이를 보는 것이 RE 루트.
...
- 20000-25000 UDP 전체 스캔 (포트는 세션별 단일 후보, 스캔은 15초 TTL 내 낮은 확률로만 성공하고 다음 단계 정보는 없음)
...
- nonce·tk는 서버 발급 1회용
- 관측값 재사용 불가(연결별 재생성)
- IP당 동시성 제한 + UDP 스페이싱으로 자동화 남발 저속화
...
| 서버 부하/GIL | sleep 오버런 |
| Docker UDP 포트 대역 매핑 | 5000개 포트 매핑 무거움 |
| Windows 로컬 ICMP port unreachable | UDP 침묵 판정 방식 차이 |
...
4. compose의 UDP 대역 매핑이 무거우면 `network_mode: host` 사용(Dreamhack VM에서 권장).
...
```
python solve\loadtest.py --workers 6 --rounds 3    # 설계 동시성: 전부 성공 (검증됨 18/18, median 3.4s)
...
- `UDP_BASE/L3*7` 계수 → transient 포트 발견 난이도
...
- [x] 단일 write probe → 220ms류 decoy + UDP 침묵 확인
- [x] token 5회 오답 → poisoned ERR 루프 확인
- [x] `sid+tk` 재사용 → ERR 확인
- [x] raw socket / CAP_* / nftables 미사용 확인
- [x] 절대 RTT·NAT 재작성 필드·커널 미정의 동작 미의존 (delta 방식)
- [x] Dreamhack 배포: `docker compose up -d` 후 외부에서 `python solve\solver.py <VM IP>`
...
```bash
cd challenge
docker compose up -d          # 또는 python server.py
# 참가자: nc <host> 9001 로 접속하면 아무것도 보이지 않는다 — 그것이 시작점이다.
```